...
class PlacoWalkEngine:
    def __init__(
        ...
        if robot_type =="open_duck_mini_v2":
            self.robot.set_joint_limits("left_knee", *knee_limits)
            self.robot.set_joint_limits("right_knee", *knee_limits)
        else:
            # ASMR

            # self.robot.set_joint_limits("leg_left_knee_pitch_joint", *knee_limits)
            # self.robot.set_joint_limits("leg_right_knee_pitch_joint", *knee_limits)
            left_foot_orientation_task = self.solver.add_orientation_task("left_foot", np.eye(3))
            left_foot_orientation_task.configure("left_foot_orientation", "soft", 1.0)
            right_foot_orientation_task = self.solver.add_orientation_task("right_foot", np.eye(3))
            right_foot_orientation_task.configure("right_foot_orientation", "soft", 1.0)
